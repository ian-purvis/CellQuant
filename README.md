...
The runnable package lives in [_cellquant/_pipeline](_cellquant/_pipeline). Installers, launch scripts, source, tests, and configs are there.

**New to CellQuant or to scientific software?** Follow [Start here](_cellquant/_pipeline/docs/START_HERE.md). It walks through installation, a first image, saving results, batch analysis, coexpression review, and the optional Alpine route without requiring code.

**Maintaining or extending the code?** Read the [developer guide](_cellquant/_pipeline/docs/DEVELOPER_GUIDE.md) for the source map, setup, tests, output contracts, and current verification limits. [Architecture](_cellquant/_pipeline/ARCHITECTURE.md) holds the detailed scientific contract. The [documentation index](_cellquant/_pipeline/docs/README.md) separates current instructions from release evidence and historical plans.
...
CellQuant 0.4.0a2 is an **alpha for lab testing**. The latest recorded software checks are dated **2026-09-09**. They do not establish biological accuracy on expert-reviewed retinal images or a live Alpine job. See the [release note](_cellquant/_pipeline/docs/RELEASE_0.4.0a2.md) and [`STATUS.json`](_cellquant/_pipeline/docs/STATUS.json) before treating results as validated scientific evidence.
