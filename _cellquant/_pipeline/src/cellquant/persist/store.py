...
class RunStore:
    ...
    def create(
        ...
        *,
        ...
        status_path = directory / _STATUS_NAME
        if status_path.is_file():
            try:
                marker = json.loads(status_path.read_text(encoding="utf-8"))
            except (OSError, json.JSONDecodeError, UnicodeDecodeError):
                marker = None
            if isinstance(marker, Mapping) and marker.get("status") == "complete":
                prior_input = marker.get("input_fingerprint")
                prior_config = marker.get("config_fingerprint")
                if prior_input != str(input_fingerprint) or prior_config != str(config_fingerprint):
                    raise FileExistsError(
                        f"refusing to reuse completed run at {directory}: fingerprints differ "
                        f"(store input={prior_input!r} config={prior_config!r}; "
                        f"requested input={str(input_fingerprint)!r} config={str(config_fingerprint)!r}). "
                        "Choose a different output path or delete the completed store explicitly "
                        "before replacing it (reviewed labels would otherwise be lost or inherited)."
                    )
        store = cls(
            directory,
            input_fingerprint,
        ...
        # A new segmentation revision must not inherit reviewed-label sidecars
        # from a prior complete/incomplete store in the same directory.
        for sidecar in ("labels_reviewed.tif", "labels_reviewed.json"):
            path = directory / sidecar
            if path.is_file():
                path.unlink()
        # Invalidate any prior completion marker before touching its event log.
        # A caller may deliberately reuse an output directory for a matching run;
