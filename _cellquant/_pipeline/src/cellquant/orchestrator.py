...
def run_pipeline(image: ImageVolume, config, cancel, events=null_event_sink):
    ...
    context = _pipeline_context(image, labels, config)
    postprocessed = _stage(
        image,
        "postprocess",
        events,
        lambda: run_postprocess(labels, config, cancel, events, context=context),
    )
    return _stage(
        image,
        "restore_grid",
...
def _pipeline_context(
    image: ImageVolume, labels: LabelVolume, config
) -> AnalysisContext:
    """Identify the analysis grid behind ``labels`` for dimension-aware stages."""

    if dict(labels.provenance.get("analysis_volume") or {}):
        return analysis_context_from_labels(labels, source=image.source)
    shape_zyx = tuple(int(value) for value in labels.data.shape)
    return analysis_context_from_config(
        config,
        original_z_depth=int(image.data.shape[0]),
        spacing_um=tuple(labels.spacing_um),
        source=image.source,
        shape_yx=shape_zyx[1:],
    )


def run_measurements(image: ImageVolume, labels, config, cancel, events=null_event_sink) -> MeasurementTables:
    """Measure using the analysis grid recorded on ``labels`` when present.

    Live segmentation settings may have changed since the mask was created.
    When ``labels`` carry ``analysis_volume`` provenance, measurement pixels are
    resolved from that grid (rejecting mismatches). Labels without provenance
    fall back to the live config and emit a warning that the grid is assumed.
    """

    provenance = dict(getattr(labels, "provenance", {}) or {}).get("analysis_volume") or {}
    if provenance:
        analysis, context = _stage(
            image,
            "analysis_grid",
            events,
            lambda: resolve_measurement_image(image, labels, cancel=cancel, events=events),
        )
    else:
        analysis = _stage(
            image,
        ...
        events(
            _event(
                ...
                "analysis_grid",
                message=(
                    "labels lack analysis_volume provenance; measuring with the live "
                    "configuration as an explicit assumption. Re-segment or declare "
                    "the grid to bind measurements to the mask plane."
                ),
            )
        )
    return _stage(
        ...
        "measure",
        events,
        lambda: measure_labels(labels, analysis, config, cancel, events, context=context),
    )
