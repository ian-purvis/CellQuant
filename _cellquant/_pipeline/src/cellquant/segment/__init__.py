...
def require_linking_stitch_threshold(value: Any) -> float:
    """Validate the ``stitch_2d`` threshold, which must actually link planes.

    Skipping ``stitch3D`` leaves plane-local Cellpose IDs untouched, so the same
    ID in two planes would be counted downstream as one object. Zero, negative,
    and above-one thresholds are therefore rejected before inference.
    """

    threshold = float(value)
    if not 0.0 < threshold <= 1.0:
        raise ValueError(
            "segment.stitch_threshold must be greater than 0 and at most 1.0 for "
            f"stitch_2d; received {threshold}. A non-positive threshold skips "
            "stitching and would merge unrelated plane-local IDs into one object. "
            "Set a linking threshold (0.25 is a good starting point) or choose "
            "another segment.mode."
        )
    return threshold
...
def load_model(spec: Mapping, device: str | None = None, events=None) -> Segmenter:
    ...
    if engine == "v3":
        ...
        custom = model_path.is_file()
        if not custom:
            if model_path.suffix or ("/" in model_name.replace("\\", "/")) or "\\" in model_name:
                raise FileNotFoundError(
                    f"custom Cellpose v3 model path does not exist: {model_name}"
                )
            if model_name not in _V3_BUILTIN_MODELS:
                raise ValueError(
                    f"unknown Cellpose v3 model {model_name!r}; "
                    f"use one of {sorted(_V3_BUILTIN_MODELS)} or an existing weight file path"
                )
    ...
    constructor_parameters["requested_model"] = model_name
    constructor_parameters["resolved_model_path"] = str(actual_path)
...
def _eval_kwargs(spec: Mapping, spacing_um: tuple[float, float, float]) -> dict[str, Any]:
    ...
    if mode == "stitch_2d":
        require_linking_stitch_threshold(spec["stitch_threshold"])
    elif float(spec["stitch_threshold"]) != 0.0:
...
def segment(
    ...
    *,
    ...
    provenance = {
        ...
        "requested_model": constructor_parameters.get("requested_model", str(spec["model"])),
        "resolved_model_path": constructor_parameters.get("resolved_model_path"),
...
__all__ = [
    ...
    "load_model",
    "require_linking_stitch_threshold",
    "segment",
    "showcase_crop",
]
