...
def _source_identity(image: ImageVolume) -> dict[str, Any]:
    """Acquisition identity a saved run needs to name its own pixels."""

    metadata = dict(image.metadata)
    keys = ("series", "series_count", "position", "position_count", "calibration_status")
    return {key: metadata[key] for key in keys if key in metadata}


def _measurement_grid_summary(
    image: ImageVolume, labels: LabelVolume, config: RunConfig
) -> str | None:
    """Describe the grid measurement will use, preferring mask provenance.

    Mirrors ``run_measurements`` so the panel names the pixels that will
    actually be measured rather than the current segmentation settings.
    """

    try:
        if dict(labels.provenance.get("analysis_volume") or {}):
            context = analysis_context_from_labels(labels, source=image.source)
        else:
            context = analysis_context_from_config(
                config,
                original_z_depth=int(image.data.shape[0]),
                spacing_um=tuple(labels.spacing_um),
                source=image.source,
                shape_yx=tuple(int(value) for value in labels.data.shape)[1:],
            )
    except (IndexError, KeyError, TypeError, ValueError):
        return None
    return analysis_context_summary(context)
...
def _validated_label_array(value: Any) -> np.ndarray:
    ...
    return data.astype(np.uint32, copy=True)
...
class PluginController:
    ...
    def __init__(
        ...
        *,
        ...
        self.analysis_context_text: str | None = None
        self.progress: tuple[int, int] | None = None
        self.batch_progress: tuple[int, int] | None = None
        self.batch_progress_status: str | None = None
    ...
    def drain_events(self, callback: Callable[[PipelineEvent], None] | None = None) -> list[PipelineEvent]:
        ...
        while True:
            ...
            if event.stage == "batch" and event.current is not None and event.total is not None:
                self.batch_progress = (int(event.current), int(event.total))
                self.batch_progress_status = str(
                    (event.details or {}).get("status") or event.kind or ""
                )
            if event.kind == "failed":
                ...
                eta = self._batch_eta_text()
                self.status_text = format_pipeline_status(
                    event,
                    eta=eta,
                    batch_progress=self.batch_progress,
                    batch_status=self.batch_progress_status,
                )
            if callback is not None:
                callback(event)
        return drained

    def _batch_eta_text(self) -> str | None:
        """ETA for the overall batch file queue, if progress is known."""

        if (
            self.batch_progress is None
            or self._job_started_monotonic is None
        ):
            return None
        current, total = self.batch_progress
        status = self.batch_progress_status or ""
        completed = int(current) if status in {"completed", "resumed"} else max(int(current) - 1, 0)
    ...
    def _dispatch(
        ...
        returned: Callable[[Any], None],
        *,
        busy_text: str = "Running in background…",
    ) -> Any:
        if self._busy:
        ...
        self.batch_progress_status = None
    ...
    def apply_segment_overrides(
        self, overrides: Mapping[str, Any] | None, *, publish: bool = True
    ) -> RunConfig:
    ...
    def run_survey_batches(
        ...
        *,
        ...
        destination = Path(output_dir)
        # Always keep survey artifacts under the output folder chosen for THIS run.
        # Reusing a stale self.survey_dir from a previous Survey wrote configs (and
        # confused follow-up UI) into the old destination after the user changed
        # the Output folder control.
        survey_dir = destination / "survey"
        self.survey_dir = survey_dir
        ...
        def returned(payload: tuple[tuple[SurveyRunResult, ...], Path]) -> None:
            ...
            try:
                ...
                # Output folder may have been moved/renamed during the run; keep
                # the in-memory config that was used for the batch.
                if self.config is None:
                    raise
            failed = sum(run.summary.failed for run in runs)
            completed = sum(run.summary.completed for run in runs)
            self.last_failures_path = None
            # Prefer the consolidated root failures.csv written by survey batches.
            consolidated = destination / "failures.csv"
            if consolidated.is_file():
                self.last_failures_path = consolidated
            else:
                for run in runs:
                    for path in failed_result_paths(run.summary):
                        if path.name == "failures.csv":
                            self.last_failures_path = path
                            break
                    if self.last_failures_path is not None:
                        break
    ...
    def segment(
        ...
        *,
        ...
        if segment_overrides:
            # Build overrides locally; only publish on successful admission.
            raw = deepcopy(dict(config.raw))
            raw["segment"] = {**raw["segment"], **dict(segment_overrides)}
            if raw["segment"].get("mode") != "stitch_2d":
                raw["segment"]["stitch_threshold"] = 0.0
        ...
        if channel_index is not None:
            ...
            raw = deepcopy(dict(local_config.raw))
            raw["preprocess"]["channel"] = channel_index
            local_config = RunConfig(raw)

        def operation():
            return self._pipeline(volume, local_config, self.cancel_token, self.enqueue_event)

        def published(labels):
            self.config = local_config
            self.image_volume = volume
            self._publish_labels(labels)

        return self._dispatch(operation, published)
    ...
    def measure_and_save(self, output_dir: str | Path, labels_layer: Any | None = None) -> Any:
        ...
        estimate = estimate_preparation(image=image, labels=getattr(layer, "data", None))
        if estimate.blocking_errors:
            self.report_exception(RuntimeError(" ".join(estimate.blocking_errors)))
            return None
        if estimate.warnings:
            self.status_text = "; ".join(estimate.warnings)
        # Snapshot on the GUI thread so later paints cannot change this save.
        # Invalid edited IDs are reported to the UI without starting a worker.
        try:
            label_array = _validated_label_array(np.array(layer.data, copy=True))
            image_snapshot = replace(
                image,
                data=np.array(image.data, copy=True),
                metadata=deepcopy(dict(image.metadata)),
            )
            provenance = {
            ...
            labels_snapshot = LabelVolume(
                label_array, image_snapshot.spacing_um, deepcopy(provenance)
            )
        except (TypeError, ValueError) as exc:
            self.report_exception(exc)
            return None
        destination = Path(output_dir)
        self.analysis_context_text = _measurement_grid_summary(
            image_snapshot, labels_snapshot, config
        )
        busy_text = (
            f"Measuring on {self.analysis_context_text} — running in background…"
            if self.analysis_context_text
            else "Running in background…"
        )

        def operation() -> Path:
            tables = self._measurements(
                image_snapshot, labels_snapshot, config, self.cancel_token, self.enqueue_event
            )
            return self._persist(destination, image_snapshot, labels_snapshot, tables, config)

        def published(path: Path) -> None:
            self.image_volume = image_snapshot
            self.label_volume = labels_snapshot
            self._on_saved(path)

        return self._dispatch(operation, published, busy_text=busy_text)

    def _on_saved(self, path: Path) -> None:
        self.last_output_dir = Path(path)
        grid = f" Measured on {self.analysis_context_text}." if self.analysis_context_text else ""
        self._publish_user_message(
            UserMessage(
                "info",
                "Saved",
                f"Run store written to {path}.{grid}",
