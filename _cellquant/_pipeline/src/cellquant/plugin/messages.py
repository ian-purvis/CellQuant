...
def format_pipeline_status(
    ...
    *,
    ...
    def _with_eta(parts: list[str]) -> str:
        if eta:
            parts.append(eta)
    ...
    if stage == "batch" and current is not None and total is not None:
        ...
        if name:
            parts.append(name)
        return _with_eta(parts)

    # Within-file progress (e.g. Z planes during stitch_2d): keep batch place
    # visible so ETA remains meaningful for the overall queue.
    if (
        stage == "segment"
        and current is not None
        and total is not None
        and batch_progress is not None
    ):
        batch_current, batch_total = batch_progress
        status = str(batch_status or "running")
        verb = "Running" if status == "running" else status.capitalize()
        parts = [f"{verb} image {batch_current} of {batch_total}"]
        if name:
            parts.append(name)
        plane_bit = str(message) if message else f"plane {current}/{total}"
        parts.append(plane_bit)
        return _with_eta(parts)

    if message:
        return _with_eta([f"{name}: {message}"] if name else [str(message)])
    label = f"{stage}: {str(kind).replace('_', ' ')}"
    if name:
        return _with_eta([f"{name}: {label}"])
    return _with_eta([label])
