...
_MODE_LABELS = {
    "volume_3d": "3D volume",
    "stitch_2d": "Linked 2D",
    "single_plane_2d": "Single analysis plane",
    "max_projection_2d": "Maximum projection",
}

_POPULATION_NOTE_NAMED = (
    "Maximum projection and Single analysis plane analyze a different slice or projection "
    "of the stack — they change which objects are counted, not just run time."
)
_POPULATION_NOTE_SHORT = (
    "Those modes analyze a different slice or projection of the stack — "
    "they change which objects are counted, not just run time."
)
...
def faster_mode_hint(current_mode: str | None) -> str:
    ...
    alternatives = [_MODE_LABELS[mode_id] for mode_id in alternative_ids]
    if not alternatives:
        return (
    ...
    population_note = ""
    if {"max_projection_2d", "single_plane_2d"} & set(alternative_ids):
        population_note = f" {_POPULATION_NOTE_SHORT}"
    return (
        f"You are currently using {label}. Faster options to try next: {joined}."
        f"{population_note}"
    )
