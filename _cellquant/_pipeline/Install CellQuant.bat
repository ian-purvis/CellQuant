@echo off
setlocal EnableExtensions
title CellQuant — install
cd /d "%~dp0"

REM Always leave this window open long enough to read errors, and keep a
REM persistent log next to this .bat (also opened in Notepad on failure).

set "LOG=%~dp0install_last.log"
set "EXITCODE=1"

echo.
echo  CellQuant installer
echo  -------------------
echo  Installs TWO environments: Cellpose-SAM v4 and classic Cellpose v3.
echo  You choose the v4 folder; v3 is created beside it as ^<name^>-v3.
echo  On NVIDIA GPUs, CUDA PyTorch is installed automatically.
echo  Press Enter at the folder prompt to use the default on this PC.
echo.
echo  If environments already exist, you will be asked to choose:
echo    [O] Open CellQuant ^(no install changes^)
echo    [U] Update existing environments in place
echo    [R] Uninstall ^(delete env folders^), then reinstall clean
echo.
echo  A full log will be written to:
echo    %LOG%
echo.

REM Optional: pass a folder as the first argument to skip the prompt.
REM Example: Install CellQuant.bat "D:\Software\cellquant-napari"
if "%~1"=="" (
  powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -File "%~dp0scripts\install_windows.ps1" -LogPath "%LOG%"
) else (
  powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -File "%~dp0scripts\install_windows.ps1" -LogPath "%LOG%" -Prefix "%~1"
)
set "EXITCODE=%ERRORLEVEL%"

echo.
if "%EXITCODE%"=="0" (
  echo  Install finished. Double-click Open CellQuant.bat and pick v3 or v4.
  echo  Paths were saved in cellquant_env.json.
  echo  Log: %LOG%
) else (
  echo  Install failed ^(exit code %EXITCODE%^).
  echo  Full details are in:
  echo    %LOG%
  echo  Opening that log in Notepad so you can read/copy the error...
  echo  Common fix: run again and press Enter for the default folder on THIS PC.
  echo  Do not use a path under another user's C:\Users\... folder.
  echo  See README.md for requirements ^(conda / Miniconda^).
  if exist "%LOG%" start "" notepad.exe "%LOG%"
)

echo.
echo  Press any key to close this window...
pause >nul
exit /b %EXITCODE%
