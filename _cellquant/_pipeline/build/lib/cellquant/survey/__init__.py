...
def with_assignments(survey: SurveyResult, assignments: Mapping[str, int]) -> SurveyResult:
    ...
    for layout in survey.layouts:
        channel = assignments.get(layout.layout_id, layout.segment_channel)
        layouts.append(
            ChannelLayout(
                layout_id=layout.layout_id,
                channel_count=layout.channel_count,
...
def run_survey_batches(
    ...
    *,
    ...
    """Run one batch queue per assigned channel layout."""
    ...
    for layout in planned.layouts:
        ...
        if token.cancelled:
            break
        config = config_for_layout(base_config, segment_channel=layout.segment_channel)
        queue = build_queue(layout.sources, destination, config)
        if not queue:
            continue
        summary = run_batch(queue, config, token, events)
        results.append(
            SurveyRunResult(
                layout_id=layout.layout_id,
                channel_names=layout.channel_names,
