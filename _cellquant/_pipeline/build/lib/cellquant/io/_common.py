...
# Friendly run-time presets used by the CLI and napari batch UI.
FILE_TYPE_PRESETS: dict[str, tuple[str, ...]] = {
    "all": SUPPORTED_SUFFIXES,
    "tiff": TIFF_SUFFIXES,
    "nd2": ND2_SUFFIXES,
}


def normalize_suffixes(suffixes: Sequence[str] | None = None) -> tuple[str, ...]:
    """Return a de-duplicated, lower-cased subset of :data:`SUPPORTED_SUFFIXES`.

    ``None`` means every supported acquisition type. An empty sequence is
    rejected so a batch never silently discovers zero file classes by mistake.
    """

    if suffixes is None:
        return SUPPORTED_SUFFIXES
    if isinstance(suffixes, (str, bytes)) or not isinstance(suffixes, Sequence):
        raise TypeError("suffixes must be a sequence of extension strings")
    if not suffixes:
        raise ValueError("suffixes must be a non-empty sequence")

    normalized: list[str] = []
    for item in suffixes:
        if not isinstance(item, str) or not item.strip():
            raise ValueError(f"invalid image suffix {item!r}")
        value = item.strip().lower()
        if not value.startswith("."):
            value = f".{value}"
        if value not in SUPPORTED_SUFFIXES:
            raise ValueError(
                f"unsupported image suffix {value!r}; expected one of {list(SUPPORTED_SUFFIXES)}"
            )
...
def resolve_file_type_preset(name: str) -> tuple[str, ...]:
    """Map a preset name (``all`` / ``tiff`` / ``nd2``) to concrete suffixes."""

    if not isinstance(name, str) or not name.strip():
        raise ValueError("file type preset must be a non-empty string")
    key = name.strip().lower()
    try:
        return FILE_TYPE_PRESETS[key]
    except KeyError as exc:
        raise ValueError(
            f"unknown file type preset {name!r}; expected one of {sorted(FILE_TYPE_PRESETS)}"
        ) from exc


def path_matches_suffixes(path: str | Path, suffixes: Iterable[str]) -> bool:
    return Path(path).suffix.lower() in {item.lower() for item in suffixes}
