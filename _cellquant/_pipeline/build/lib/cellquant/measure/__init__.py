...
OBJECT_COLUMNS = [
    "label", "voxel_count", "volume_um3", "centroid_z_px", "centroid_y_px", "centroid_x_px",
...
def measure_labels(labels: LabelVolume, image: ImageVolume, config=None, cancel=None, events=None) -> MeasurementTables:
    ...
    if ids.size == 0:
        ...
        return MeasurementTables(
            pd.DataFrame(columns=OBJECT_COLUMNS), pd.DataFrame(columns=intensity_columns)
        )
    if cancel is not None:
        cancel.raise_if_cancelled()
    spacing = np.asarray(labels.spacing_um, dtype=float)
    voxel_um3 = float(np.prod(spacing))
    ...
    objects.attrs["units"] = {
        "volume_um3": "um^3", "centroid_*_um": "um", "bbox_*_um": "um",
...
__all__ = ["MeasurementTables", "measure_labels", "showcase_crop", "write_measurements"]
