...
class ClassificationRecipe:
    ...
    def __init__(self, raw):
        ...
        if queries is None:
            queries = [dict(name=" & ".join(f"{n}+" for n in subset), positive=list(subset))
                       for size in range(1, len(names)+1) for subset in combinations(names, size)]
...
def classify_labels(image: ImageVolume, labels: LabelVolume, recipe, *, region=None, context=None, cancel=None):
    ...
    for marker in spec["markers"]:
        ...
        if channel is not None:
            ...
            np.divide(np.bincount(groups, weights=within, minlength=len(ids)), counts,
                      out=fraction, where=valid & (counts > 0))
            cutoff, margin = marker["positive_fraction"], marker["uncertainty_margin"]
            calls[valid] = "uncertain"
            calls[valid & (fraction < cutoff-margin)] = "negative"
            calls[valid & (fraction >= cutoff+margin)] = "positive"
